...
bool XGUnit::init(XGPlayer* Player, CCPoint& Pos, const char* Texture)
{
	bool ret = false;
	do 
	{
...
XGUnit* XGUnit::create(XGPlayer* Player, CCPoint& Pos, const char* Texture)
{
	XGUnit* unit = new XGUnit();
	if(unit && unit->init(Player, Pos, Texture))
	{
		unit->autorelease();
		return unit;
	}
