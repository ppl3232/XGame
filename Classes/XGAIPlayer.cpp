...
void XGAIPlayer::AITacticsSimple(XGUnit* Unit)
{
	...
	else
	{
		...
		if(target != NULL)
		{
			XGTilePoint pos = FindClosestPointWithTarget(Unit, target);
			Unit->ActionMove(pos);
...
XGTilePoint XGAIPlayer::FindClosestPointWithTarget(XGUnit* Unit, XGUnit* Target)
{
	int MinDis = 1000;
	XGTilePoint pos;
	CCArray* MoveableTiles = Unit->GetMoveableTiles();
	...
	return XGTilePoint(pos.x, pos.y);
}
